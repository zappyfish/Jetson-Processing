...
final_jetson_system::final_jetson_system() : m_is_armed(false), m_lpf_accel(0.05), m_should_sample_accel(true),
...
void final_jetson_system::beacon_deployed_callback(const char *name, std::vector<const char *> keys,
    ...
    system->m_nrf_handler.send_packet(packet.get_x(), packet.get_y());
